...
:: =============================================================================
:: RunWhatsApp.bat - Versao corrigida
:: Correcoes:
::   1. Bootstrap log ANTES de qualquer logica (garante rastreabilidade)
...
::   5. Limpeza de sessao corrompida ao detectar reauth request (ExitCode=21)
:: =============================================================================
...
:: ---------------------------------------------------------------------------
:: BOOTSTRAP LOG - Primeiro registro, ANTES de qualquer call ou validacao.
:: 2>nul garante que falha de escrita (arquivo aberto no editor) NAO aborta o BAT.
:: ---------------------------------------------------------------------------
...
:: ---------------------------------------------------------------------------
:: Garantir diretorio de trabalho correto
:: ---------------------------------------------------------------------------
cd /d "%BASE_DIR%"
...
:: Se Node solicitou reautenticacao interativa (ExitCode=21)
if "!NODE_EXIT!"=="!REAUTH_EXIT!" (
    call :log REAUTH: Node exigiu reautenticacao. ExitCode=!REAUTH_EXIT!

    :: Deletar sessao corrompida para forcar re-pareamento limpo
    if exist "%SESSION_DIR%\" (
...
:: 2>nul em TODAS as escritas para nunca abortar quando o arquivo esta travado.
set "TS="
