...
def process() -> None:
    ...
    try:
        ...
        df_last = pd.DataFrame(last_state)
        diff_rows = []
        ...
        for _, row in df_agreg.iterrows():
            ...
            match = df_last[df_last['Key'] == row['Key']]
            if match.empty:
                row_diff = row.copy()
                row_diff['_change_type'] = 'NEW'
                diff_rows.append(row_diff)
        ...
        if not diff_rows:
            log("Sem alteracoes relevantes desde o ultimo ciclo. Notificacao suprimida.", "INFO", exec_id)
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            df_agreg_final.to_excel(writer, sheet_name='ReceitasBloqueadas', index=False)
            df_raw.to_excel(writer, sheet_name='OBsReceitasBloqueadas', index=False)
        
        try:
            formatar_excel(excel_path)
            log("Excel formatado com sucesso.", "INFO", exec_id)
        except Exception as e:
            log(f"Erro ao formatar Excel: {e}", "WARN", exec_id)
            
        df_diff = pd.DataFrame(diff_rows)
        html_content = gerar_html_artistico(df_agreg, df_diff, stats)
        with open(os.path.join(os.path.dirname(__file__), "email_body.html"), "w", encoding="utf-8") as f:
            f.write(html_content)
            
        log("Processo concluido. Mudancas prontas para envio.", "INFO", exec_id)

    except oracledb.DatabaseError as de:
        error_obj, = de.args
        log(f"Erro de Banco de Dados: {error_obj.message}", "ERROR", exec_id)
        sys.exit(1)
    except Exception as e:
