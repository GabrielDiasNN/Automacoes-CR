...
- **Onda 4 EM PROGRESSO**, dividida em 3 fases (checkpoint de paleta do plano exige aprovação humana antes de tocar componente com cor nova):
  ...
  - **4-2 ✅ FECHADA E VERIFICADA** — a11y restante, responsivo, fontes self-hosted, breakpoint único (gate `@media`).
  - **4-3 ✅ FECHADA E VERIFICADA** — tema claro (paleta aprovada), `ThemeContext`, auditoria de acessibilidade automatizada.
- **Onda 4 (as 3 fases) FECHADA.** Rodada 2 completa: Ondas 1, 2, 3, 4, 5, 6 todas ✅ VERIFICADAS E FECHADAS.

### Onda 4-3 — tema claro, ThemeContext, auditoria de a11y (branch `escalar/frontend-rodada2-onda4`, de 4-2)
- **Checkpoint humano (a) aprovado nesta sessão** — proposta de paleta/tipografia publicada como Artifact ("Painel de Instrumentos Claro"), aprovada pelo usuário antes de qualquer commit em componente.
- [x] `831c959` — `tokens.css` reestruturado: `:root` continua definindo o escuro (byte-idêntico); `[data-theme="light"]` + espelho em `@media (prefers-color-scheme: light)` redefinem só a camada semântica. `--scrim-color` não muda entre temas (decisão: overlay de modal fica escuro nos dois). `ThemeContext` (`system|light|dark`, mesmo padrão do `TableDensityContext`) + botão no topbar (Shell.tsx, ciclo Monitor/Sun/Moon). `chartPalette.ts`: grid do uPlot lia `--graphite-700` direto (nunca mudaria de tema) — trocado para `--border`.
- [x] `de9804b` — 4 baselines de screenshot claros (Painel/Automações/Monitor/Beneficiamento), commit isolado revisado a olho. **Achado:** os 4 testes escuros nunca fixavam tema (dependiam do `prefers-color-scheme` do ambiente, nunca testado antes porque só havia 1 tema) — Playwright emula `light` por padrão, o que teria virado os 4 baselines escuros pra claro. Fix: `_inject_theme(page, "dark")` explícito nos 4 testes escuros. **Achado 2:** baseline de Automações capturou o skeleton (corrida entre o heading e os cards) — `wait_for_selector` adicional.
- [x] `4a2ec12` — **auditoria de acessibilidade automatizada** (axe-core, não jest-axe/RTL — o CLAUDE.md já veta testar `pages/` em bloco com RTL). axe-core como devDependency do Dashboard, injetado via `page.route` (CSP `script-src 'self'` bloqueia `add_script_tag(path=...)`, que embute inline). 4 testes novos (Painel/Automações/Monitor/Beneficiamento), bloqueando em impact serious/critical. Achou 3 violações reais, **nenhuma exclusiva do tema claro** (dívida pré-existente, só nunca auditada): `Annunciator.off` com `opacity:0.7` amassando texto no piso AA (falhava nos DOIS temas); `DataTable` scroll horizontal não alcançável por teclado; `AutomacoesPage` `<dl>` malformado (`<div>` fora do par dt/dd).
- [x] `2b146a1` — **achado 4, o mais substancial:** a primeira leva de sinais claros só garantia contraste contra a superfície pura; `StatusTag` usa a mesma cor como texto E fundo (tint), e no claro (texto perto do piso) isso reprovava AA em 4,0–4,3:1 em 5 badges/3 telas. Os 6 sinais recalculados de novo, agora com margem para o próprio tint no pior caso.
- [x] `463b464` — baselines claros atualizados após o recálculo de cor (commit isolado, revisado a olho de novo).
- **Verificação final: lint 0/0 · typecheck 0 · 183 vitest · bandit -ll limpo · mypy+pylint limpo · build isolado (index 23,99 KB gzip) · 25/25 E2E (17 originais + 4 screenshots claros + 4 a11y) · 4 baselines escuros intactos desde a 4-2 · viewport 360px sem overflow nas 4 telas com tema claro · evidência registrada via `Test-PlaywrightEvidence.ps1`.**

### Onda 4-2 — a11y remanescente, responsivo, fontes, breakpoint único (branch `escalar/frontend-rodada2-onda4`, de 4-1)
- [x] a11y `e16a337` — Toast (role duplicado + WCAG 2.2.1 pausa no hover), ConfirmModal (`aria-labelledby`), CommandPalette (`role="combobox"` + listbox correto + outline), ApiKeyGate (outline), DataTable/ClickableTile (Space além de Enter), AutomacoesPage (`aria-label` no motivo de desabilitado). Investigado sem violação real: "zero `<label>`" (todo input já tem `aria-label`) e "`title=` em 41 lugares" (maioria é prop de componente que vira `<h3>` visível, não o atributo HTML).
...
- [x] fontes self-hosted `6c81a65` — **checkpoint humano aprovado nesta sessão** (download de arquivo exige confirmação explícita). 10 woff2 (Sans 400/500/600/700, Sans Condensed 500/600/700, Mono 400/500/600, subset latin) baixados diretamente do Google Fonts (mesma fonte de origem, só muda o transporte) para `src/assets/fonts/` — **não** `public/fonts/`: com `base: "/dashboard/"` no `vite.config.ts`, um `url()` absoluto em `public/` não ganharia o prefixo; como asset processado pelo Vite, o caminho relativo é reescrito certo em dev e build (confirmado no CSS gerado: `/dashboard/assets/ibm-plex-*-HASH.woff2`). Achado no caminho: a API `css2` do Google Fonts serve um arquivo variável único (mesma URL para os 4 pesos) quando o User-Agent é um browser moderno — precisou de UA legado (Chrome 60, pré-variable-font) pra obter os 10 arquivos estáticos corretos, senão os 4 pesos de "IBM Plex Sans" renderizariam idênticos.
- [x] gate `@media` — grep de fechamento: só os 4 valores de `breakpoints.ts` (400/560/720/900) + a exceção documentada de 721px (`FilterBar`, evita zona morta de 1px) + `prefers-reduced-motion`. Zero literal órfão.
- [x] verificação final da 4-2: lint 0/0 · typecheck 0 · 180 vitest · build isolado medido (index 23,69 KB gzip, CSS 34,02 KB gzip com as fontes embutidas) · **17/17 E2E, 4 screenshots sem regenerar baseline** em cada um dos 3 commits · fontes conferidas no browser real (200 OK nos 10 arquivos, sem erro de console).
