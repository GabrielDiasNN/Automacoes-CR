...
- [x] **F2-T2** — Setup Vitest + Testing Library implementado (`Dashboard/vitest.config.ts`, scripts `test`/`test:watch`/`test:coverage`, job `frontend` do CI). **Escopo corrigido**: os 5 arquivos do plano original (`schedule.parser`, `auth.localstorage`, `diagnostics.badges` CAT/DRIFT/DOCS, `action.codes`, `beneficiamento.kpi`) não correspondem a código real do Dashboard — não há parser de schedule no frontend (o backend já manda `schedule_summary` pronto), não há badges CAT/DRIFT/DOCS nem conceito de `action_code`, e a API key usa `sessionStorage` (não `localStorage`, sem limpeza automática no 403). Substituído por 4 arquivos cobrindo a lógica real e testável: `format.test.ts` (20 cenários de `lib/format.ts`), `status.test.ts` (22 cenários do mapeamento de tom/badge em `lib/status.ts`), `client.test.ts` (`qs`, `setApiKey`/`getApiKey`, tratamento de erro em `api/client.ts`) e `useApiKey.test.ts` (persistência real via `sessionStorage`). 54 testes, todos passando; `tsc --noEmit`, ESLint e `vite build` continuam limpos.
