...
- **Governança de datas e guardrail de não regressão (26/05/2026)**: instituído o formato brasileiro `DD/MM/YYYY` como padrão de exibição, logs textuais e documentações Markdown ativos do repositório. O novo script `Tools/Test-DateConformidade.ps1` foi integrado ao Quality Gate para bloquear automaticamente novos commits que reintroduzam datas ISO nestes locais.
