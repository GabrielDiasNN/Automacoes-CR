...
| 15 | `Test-LogEventSchema.ps1` | Eventos fora de `docs/log-event.schema.json` (ver `docs/logging-standard.md`). **`blocking`**: valida `docs/log-event.samples.jsonl` (golden, modo estrito — âncora do CI) e `Logs/*.jsonl` local em `--rollout` (ignora linha legada sem `trace_id`). |
