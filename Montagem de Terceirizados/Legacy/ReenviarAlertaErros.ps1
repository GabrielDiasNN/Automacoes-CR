...
#
# USO:
#   pwsh -File ".\Montagem de Terceirizados\ReenviarAlertaErros.ps1"
#   pwsh -File ".\Montagem de Terceirizados\ReenviarAlertaErros.ps1" -EmailPreviewOnly -EmailToTest "homolog@empresa.com.br" -EmailCcTest "qa@empresa.com.br"
#
# QUANDO USAR:
...
$BasePath = ".\Montagem de Terceirizados"
...
function Write-Log {
    ...
    Write-Host $line
    try { Add-Content -Path $LogFile -Value $line -Encoding UTF8 } catch [System.Exception] {}
}
...
if (Test-Path $LogFile) {
    try { $logOffsetBefore = (Get-Item $LogFile).Length } catch [System.Exception] {}
}
