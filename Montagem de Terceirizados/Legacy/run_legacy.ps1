...
# ???? Integracao com a variavel global de Modo de Teste


$globalTestEmail = [Environment]::GetEnvironmentVariable("AUTOMACAO_TEST_EMAIL", "User")


if (-not [string]::IsNullOrWhiteSpace($globalTestEmail)) {


    Write-Log "TRAVA DE SEGURANCA: Modo de teste global detectado ($globalTestEmail). Aplicando redirecionamento..." -Lvl "WARN"


    $EmailToTest = $globalTestEmail


    $EmailCcTest = "" # Limpa CC para evitar envios em duplicidade no teste


    $env:AUTOMACAO_TEST_EMAIL = $globalTestEmail # Forca para o ambiente do Excel
...
try {
    ...
    if ($temOverrideEmail) {
        ...
        Write-Log "Modo de notificacao TESTE ativo (via Ambiente). O VBA lidara com o redirecionamento via AUTOMACAO_TEST_EMAIL."


        if (-not [string]::IsNullOrWhiteSpace($EmailToTest)) {


            [System.Environment]::SetEnvironmentVariable("AUTOMACAO_TEST_EMAIL", $EmailToTest, "Process")
