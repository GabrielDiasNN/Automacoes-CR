...
Public Function ProcessarNotificacoesWrapper(ByRef udtTel As Telemetria, ByRef arrErros() As DadosErro, ByVal blnSilencioso As Boolean) As Boolean
    ...
    ' 1. Tentativa de Notificacao Customizada (Dynamics/WhatsApp/Etc)
    ' O modNotificacoesCustom deve implementar ProcessarNotificacoesCustomizadas
    On Error Resume Next
    modNotificacoesCustom.ProcessarNotificacoesCustomizadas udtTel.totalLinhas, udtTel.totalErros, arrErros, blnSilencioso
    
    If Err.Number = 0 Then
        ProcessarNotificacoesWrapper = True
    Else
        GravarLogEx "Aviso: Notificacao customizada falhou ou nao existe: " & Err.Description, LOG_WARNING
    End If
    On Error GoTo 0
End Function
