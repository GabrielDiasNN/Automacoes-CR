...
' VUL-11: constantes de layout Do Dashboard.
' DASH_HEADER_ROWS deve refletir a ultima linha Do cabecalho (linha 9 = colnames).
' DASH_MAX_REGISTROS define quantas linhas de historico sao mantidas (50 ultimas).
Private Const DASH_HEADER_ROWS   As Long = 9
Private Const DASH_MAX_REGISTROS As Long = 50
...
Public Sub RegistrarHistorico(ByVal blnSucesso As Boolean, ByRef udtTel As Telemetria)
    ...
    On Error GoTo Falha
        ...
        On Error GoTo Falha
            ...
            If lngUltima > DASH_HEADER_ROWS + DASH_MAX_REGISTROS Then
                ' VUL-11: remove a linha de dado mais antiga (primeira linha de dado)
                ' usando constante para nao depender de numero magico hardcoded.
                objWs.Rows(DASH_HEADER_ROWS + 1).Delete
                lngUltima = DASH_HEADER_ROWS + DASH_MAX_REGISTROS
            End If
            ...
            With objWs
                .Cells(lngUltima, 1).Value = FormatarDataBR(Now, True)
