...
#   "description": "Nucleo de validacao NF/OB com tipagem estrita",
...
def processar_validacao(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in data:
        ...
        if not montagem_ok and not prog_ok:
            detalhe_erro = "Erro de Montagem e Programacao"
        elif not montagem_ok:
            detalhe_erro = "Erro de Montagem"
        elif not prog_ok:
            detalhe_erro = "Erro de Programacao"
...
def gerar_tabela_completa_erros(erros: List[Dict[str, Any]]) -> str:
    ...
    if not erros:
        return ""
    html: str = (
        "<div style='margin-top: 30px;'><h3 style='font-size:14pt; margin:0 0 12px 0; color:#1f2937; text-align:center;'>Detalhamento Completo das Divergencias</h3>"
    )
...
def montar_template_email(
    ...
    msg: str = "Divergencias Detectadas"
    ...
    if tipo == "ERRO":
        html += f"<p style='font-size:13pt;margin:0 0 8px 0;text-align:center;color:#374151;'><span style='font-size:16pt;'>{HTML_ICON_MAGNIFY}</span> <b>Foram detectadas divergencias entre a Montagem e a Programacao.</b></p>"
    elif tipo == "ALTERACAO":
        html += f"<p style='font-size:13pt;margin:0 0 8px 0;text-align:center;color:#374151;'><span style='font-size:16pt;'>{HTML_ICON_CHART_UP}</span> <b>Houve uma atualizacao no status das divergencias acompanhadas.</b></p>"
    else:
        html += f"<p style='font-size:13pt;margin:0 0 12px 0;text-align:center;color:#374151;'><span style='font-size:16pt;'>{HTML_ICON_TROPHY}</span> <b>Nenhuma divergencia foi encontrada na rotina atual. O processo esta liberado.</b></p>"
...
def main() -> None:
    ...
    if tipo_notif != "NENHUMA":
        ...
        detalhes_html: str = ""
        subject: str = (
            f"Alerta: Divergencias Montagem - {datetime.now().strftime('%d/%m/%Y')}"
        )

        if tipo_notif == "ERRO":
            detalhes_html = gerar_tabela_completa_erros(erros_atuais)
        elif tipo_notif == "ALTERACAO":
            subject = f"Alteracao: Divergencias Montagem - {datetime.now().strftime('%d/%m/%Y')}"
            ...
            subject = (
                f"Sucesso: Validacao Aprovada - {datetime.now().strftime('%d/%m/%Y')}"
            )
            detalhes_html = ""
        ...
        try:
            ...
            sys.exit(1)
        log(f"Processamento concluido ({tipo_notif}).", "INFO", exec_id)
    else:
