...
function extrairOpcoesMensagem(caption) {
    const options = { caption };
    if (caption && typeof caption === 'string') {
        const matches = [...caption.matchAll(/@(\d+)/g)].map(m => `${m[1]}@c.us`);
        if (matches.length > 0) {
            options.mentions = matches;
            writeLog('INFO', `Detectadas ${matches.length} mencoes para o envio: [${matches.join(', ')}]`);
        }
    }
    return options;
}
        ...
        client.on('ready', async () => {
            try {
                ...
                if (ATTACHMENT_PATH && fs.existsSync(ATTACHMENT_PATH)) {
                    ...
                    sentMsg = await client.sendMessage(chatId, media, extrairOpcoesMensagem(CAPTION));
                } else {
                    writeLog('WARN', 'Anexo ausente ou nao encontrado. Enviando apenas a mensagem de texto.');
                    sentMsg = await client.sendMessage(chatId, CAPTION, extrairOpcoesMensagem(CAPTION));
                }
...
async function executarFluxoBatch(client, itens, chatId, bootstrapAttempt) {
    ...
    for (const item of itens) {
        ...
        try {
            ...
            try {
                const sentMsg = await client.sendMessage(chatId, media, extrairOpcoesMensagem(caption));
                ...
                if (acked) {
                    ...
                    try {
                        const retryMsg = await client.sendMessage(chatId, media, extrairOpcoesMensagem(caption));
