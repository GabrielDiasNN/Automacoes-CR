...
//   "logging": "structured-events (HUB_LOG_STRUCTURED); ver docs/logging-standard.md"
// }
const fs = require('fs');
...
const { maskSensitive } = require('./log-masking');
const {
...
// O whatsapp-web.js 1.34.x pode devolver undefined mesmo depois de o evento
// message_create identificar a mensagem local. O prazo cobre o envio assíncrono
// sem deixar um processo preso indefinidamente quando não há confirmação.
const DISPATCH_EVENT_TIMEOUT_MS = 30000;
// LIST_GROUPS: o whatsapp-web.js 1.34.7 removeu window.Store (usado ate' a 1.34.6 para
// enumerar chats direto). O substituto interno da propria lib — window.WWebJS.getChats(),
// que chama window.require('WAWebCollections').Chat.getModelsArray() — reproduz o mesmo
// TypeError em sessoes autenticadas ha' mais tempo (upstream aberto e sem fix: issues
// wwebjs/whatsapp-web.js #201845 e #5733, "r: r"/"t: t" minificado). Sem alternativa
// limpa de enumeracao instantanea, o modo cai para captura por eventos de protocolo
// (message_create + notificacoes de grupo), que nao passam pelo Store/Collections.
const LIST_GROUPS_COLLECTIONS_TIMEOUT_MS = 5000;
const LIST_GROUPS_CAPTURE_MS = parseInt(process.env.LIST_GROUPS_CAPTURE_MS || '20000', 10);
...
// Padrao de logging estruturado (docs/logging-standard.md). Com HUB_LOG_STRUCTURED=1
// (exportado pelo run.ps1 da automacao ja migrada) cada linha e um evento JSON no
// envelope do schema; sem a env mantem o formato legado [NODE-WA].
const LOG_ESTRUTURADO = ['1', 'true', 'True'].includes((process.env.HUB_LOG_STRUCTURED || '').trim());
const HUB_AUTOMATION = (process.env.HUB_AUTOMATION || '').trim() || 'WhatsApp';
const HUB_TRACE_ID = (process.env.HUB_TRACE_ID || '').trim()
    || `wa-${new Date().toISOString().replace(/[-:]/g, '').replace(/\.\d+Z$/, 'Z')}-${Math.floor(Math.random() * 0x10000).toString(16).padStart(4, '0')}`;
const HUB_STEP = (process.env.HUB_STEP || '').trim() || 'dispatch';
...
function _nivelSchema(nivel) {
    const up = String(nivel || 'INFO').toUpperCase();
    if (up === 'ERROR' || up === 'ERRO') return 'ERRO';
...
function writeLog(nivel, mensagem) {
    ...
    if (LOG_ESTRUTURADO) {
        linha = JSON.stringify({
            ts: _tsUtc(),
            level: _nivelSchema(nivel),
            component: 'node_whatsapp',
            event: 'log',
            automation: HUB_AUTOMATION,
            exec_id: EXEC_ID,
            trace_id: HUB_TRACE_ID,
            message: maskSensitive(mensagem),
            step: HUB_STEP
        });
    } else {
        linha = `[${agoraBR()}] [NODE-WA] [${nivel}] [ExecId:${EXEC_ID}] ${mensagem}`;
    }
    console.log(linha);
    // No padrao estruturado o stdout e o canal unico: o run.ps1 pai persiste a
    // linha no .jsonl da automacao via Write-HubForwardedLine. Um append proprio
    // aqui gravaria cada evento duas vezes no mesmo arquivo.
    if (LOG_ESTRUTURADO) return;
    if (!LOG_FILE) return;
...
async function resolverMencoes(client, caption) {
    ...
    // `waitUntilMsgSent` NAO entra aqui: e' opcao de transporte de sendMessage, sem
    // relacao com mencoes — quem precisa dela (enviarComConfirmacaoDeDispatch) a adiciona.
...
function shouldRetryInitialize(error) {
    if (error && error.dispatchTentado) {
        // Erro ocorreu depois que o dispatch da mensagem real ja comecou: retentar
        // criaria um cliente novo e reenviaria a mensagem sem saber se a tentativa
        // anterior chegou a ser entregue. O retry existe para falha de bootstrap
        // (antes do 'ready'), nao para falha durante o envio.
        return false;
    }
...
function normalizarCorpoMensagem(valor) {
    if (typeof valor !== 'string') {
        return '';
    }
    // NFC + colapso de espacos internos: o corpo devolvido pelo servidor no
    // message_create pode normalizar acentuacao ou espacamento de forma diferente
    // do texto originalmente enviado — sem isso, uma mensagem fisicamente entregue
    // seria rejeitada por divergencia cosmetica do body.
    return valor.normalize('NFC').replace(/\r\n?/g, '\n').replace(/[ \t]+/g, ' ').trim();
}

function serializarIdMensagem(id) {
    ...
    // Na versão instalada do WhatsApp Web, o modelo exposto pelo
    // whatsapp-web.js perde `_serialized`, mas preserva o valor no campo
    // interno `$1` (observado também nos eventos message_create/message_ack).
    if (typeof id.$1 === 'string' && id.$1) {
        return id.$1;
    }
    if (id.fromMe === true && typeof id.remote === 'string' && typeof id.id === 'string') {
        const participante = typeof id.participant === 'string' ? `_${id.participant}` : '';
        return `true_${id.remote}_${id.id}${participante}`;
    }
    return null;
}

// A string serializada de uma mensagem propria em grupo pode vir COM ou SEM o sufixo de
// participante conforme a origem do evento: o `_serialized` que o WhatsApp Web devolve no
// message_ack normalmente nao o traz, enquanto o fallback construido acima (usado quando o
// modelo perde `_serialized`/`$1`) sempre traz. Comparar a string inteira faria o ACK ser
// descartado por divergencia de sufixo; a identidade real da mensagem e' `remote`+`id`.
function chaveIdMensagem(idSerializado) {
    if (typeof idSerializado !== 'string' || !idSerializado) {
        return null;
    }
    const partes = idSerializado.split('_');
    return partes.length >= 3 ? `${partes[1]}_${partes[2]}` : idSerializado;
}

// Ids de mensagem ja adotados por um dispatch neste processo. Guarda ESTRUTURAL contra
// adotar o mesmo message_create duas vezes: substitui a antiga tolerancia de relogio de
// 5s, que nao rejeitava nenhum dos dois casos que existia para cobrir — o warmup ocorre
// ~2s antes do dispatch e a pausa entre fases do lote e' de 3s, ambos DENTRO da janela.
// Sem depender de relogio, tambem e' imune a deriva de timestamp do servidor.
//
// Nunca podado (so' `_resetIdsConsumidosForTests`, exclusivo de teste): pressupoe
// processo de vida curta, uma execucao de lote por vez (no maximo ~18 ids por lote
// hoje). Se este motor passar a rodar como servico de longa duracao, este Set precisa
// de poda por tempo/tamanho.
const IDS_DISPATCH_CONSUMIDOS = new Set();

function registrarIdConsumido(idSerializado) {
    if (typeof idSerializado === 'string' && idSerializado) {
        IDS_DISPATCH_CONSUMIDOS.add(idSerializado);
    }
}

function _resetIdsConsumidosForTests() {
    IDS_DISPATCH_CONSUMIDOS.clear();
}

// O WhatsApp Web migrou o enderecamento de chats individuais para LID: uma mensagem
// despachada para "<numero>@c.us" volta no message_create com _getChatId() = "<lid>@lid".
// Comparar so' com o numero discado rejeitava TODO dispatch para contato individual —
// e como nesta build sendMessage devolve undefined (o Store.MsgKey perdeu `_serialized`,
// e o whatsapp-web.js 1.34.6 faz `Store.Msg.get(key._serialized)`), message_create e' o
// UNICO caminho de confirmacao. Dai o falso negativo de RB-01 desde 25/08/2026: mensagem
// entregue (ack=3) e execucao encerrada com ExitCode=24.
// Continua fail-closed: aceita apenas os enderecos comprovadamente do MESMO chat
// (o discado e o canonico resolvido por resolverDestinosDoChat), nunca "qualquer @lid".
function normalizarDestinos(destinos) {
    if (destinos instanceof Set) {
        return destinos;
    }
    if (Array.isArray(destinos)) {
        return new Set(destinos.filter(Boolean));
    }
    return destinos ? new Set([destinos]) : new Set();
}

// Resolve o endereco canonico do chat (que pode vir em @lid) alem do numero discado.
// Best-effort: se o Store nao expuser o chat, o conjunto fica so' com o discado e o
// comportamento volta a ser o estrito — falha explicita, nunca falso positivo.
async function resolverDestinosDoChat(client, chatId) {
    const destinos = new Set([chatId]);
    try {
        const canonico = await client.pupPage.evaluate(async (alvo) => {
            const chat = await window.WWebJS.getChat(alvo, { getAsModel: false });
            return chat && chat.id && chat.id._serialized ? chat.id._serialized : null;
        }, chatId);
        if (canonico && canonico !== chatId) {
            destinos.add(canonico);
            // Loga so' o dominio: o identificador completo e' dado do destinatario e o
            // mascaramento de numeros so' acontece na camada PowerShell.
            writeLog('INFO', `Destino canonico do chat difere do numero discado (dominio @${String(canonico).split('@')[1]}) — ambos aceitos na confirmacao de dispatch.`);
        }
    } catch (error) {
        writeLog('WARN', `Nao foi possivel resolver o endereco canonico do chat: ${describeError(error)}. Seguindo apenas com o numero discado.`);
    }
    return destinos;
}

function extrairIdMensagemCriada(msg, chatId, corpoEsperado) {
    const id = serializarIdMensagem(msg && msg.id);
    if (!id) {
        return null;
    }
    const fromMe = msg.fromMe === true || (msg.id && msg.id.fromMe === true);
    if (!fromMe) {
        return null;
    }
    const destino = typeof msg._getChatId === 'function'
        ? msg._getChatId()
        : (typeof msg.to === 'string' ? msg.to : msg.to?._serialized);
    const aceitos = normalizarDestinos(chatId);
    // Fail-closed: se o chat de origem nao pode ser determinado, rejeita em vez de
    // aceitar — aceitar por padrao permitiria que um message_create de OUTRO chat
    // fosse casado como confirmacao do dispatch atual.
    if (aceitos.size > 0 && !aceitos.has(destino)) {
        return null;
    }
    const esperado = normalizarCorpoMensagem(corpoEsperado);
    if (esperado) {
        if (normalizarCorpoMensagem(msg.body) !== esperado) {
            return null;
        }
    } else if (IDS_DISPATCH_CONSUMIDOS.has(id)) {
        // Sem corpo esperado (ex.: midia sem legenda) para desambiguar, fromMe+chatId
        // sozinhos casam tambem com o warmup ou com um message_create atrasado de uma
        // fase anterior do lote. Como cada id ja adotado (inclusive o do warmup) fica
        // registrado, repetir um deles identifica o evento antigo sem consultar relogio.
        return null;
    }
    return id;
}

function observarMensagemCriada(client, destinosAceitos, corpoEsperado) {
    let concluido = false;
    let timer;
    let resolver;
    ...
    const listener = msg => {
        const id = extrairIdMensagemCriada(msg, destinosAceitos, corpoEsperado);
        if (id) {
            finalizar({ id, msg });
        }
    };
    client.on('message_create', listener);
    // O timeout so' e' armado por armarTimeout(), chamado depois que o await
    // sendMessage(...) resolver: envio de midia com waitUntilMsgSent pode levar
    // bem mais que DISPATCH_EVENT_TIMEOUT_MS, e armar o timer antes do envio
    // descartava o listener antes do message_create correspondente chegar.
    return {
        promise,
        armarTimeout: () => {
            if (concluido) return;
            timer = setTimeout(() => finalizar(null), DISPATCH_EVENT_TIMEOUT_MS);
        },
        cancelar: () => finalizar(null)
    };
}

// `chatId` e' o endereco de ENVIO (o numero discado); `destinosAceitos` e' o conjunto de
// enderecos que identificam o mesmo chat no message_create (inclui o @lid canonico).
async function enviarComConfirmacaoDeDispatch(client, chatId, content, options, destinosAceitos) {
    const corpoEsperado = typeof content === 'string' ? content : options?.caption;
    const observacao = observarMensagemCriada(client, destinosAceitos || chatId, corpoEsperado);
    try {
        // Aguarda o resultado do servidor antes de seguir. Sem esta opção, o
        // whatsapp-web.js 1.34.x pode resolver sendMessage enquanto a mensagem
        // ainda esta apenas na fila local — o proprio motivo de existir desta
        // funcao (confirmar o dispatch, nao so' o enfileiramento).
        const sentMsg = await client.sendMessage(chatId, content, { ...options, waitUntilMsgSent: true });
        observacao.armarTimeout();
        const retornoId = serializarIdMensagem(sentMsg && sentMsg.id);
        if (retornoId) {
            registrarIdConsumido(retornoId);
            return { sentMsg, msgId: retornoId, origem: 'retorno' };
        }
        const evento = await observacao.promise;
        if (evento) {
            registrarIdConsumido(evento.id);
            writeLog('WARN', `sendMessage retornou sem id; message_create confirmou o dispatch (${evento.id}).`);
            return { sentMsg: evento.msg, msgId: evento.id, origem: 'message_create' };
        }
        return { sentMsg: null, msgId: null, origem: 'nenhuma' };
    } finally {
        observacao.cancelar();
    }
}

// Com `waitUntilMsgSent`, sendMessage so' resolve depois que o servidor aceitou a
// mensagem — que e' exatamente quando o ack=1 e' emitido. O ACK chega, portanto, na
// janela em que `targetMsgId` ainda e' nulo (ele so' e' gravado depois do retorno do
// dispatch). Registrar todo ACK recebido num Set e consultar esse Set ao definir o
// alvo evita descartar sistematicamente o ACK e queimar a janela inteira de espera.
function registrarAckRecebido(state, msgId, ack) {
    const chave = chaveIdMensagem(msgId);
    if (!chave || !(ack >= 1)) {
        return false;
    }
    state.acksRecebidos.add(chave);
    if (state.targetMsgId && chaveIdMensagem(state.targetMsgId) === chave) {
...
function definirTargetMsgId(state, msgId) {
    state.targetMsgId = msgId;
    const chave = chaveIdMensagem(msgId);
    if (chave && state.acksRecebidos.has(chave)) {
        state.ackConfirmado = true;
        return true;
    }
    return false;
...
async function aguardarAck(client, state) {
    if (!state.targetMsgId) {
        // Guarda defensiva: o dispatch sem retorno nem message_create é rejeitado
        // antes desta função. Nunca promover esse caminho a sucesso.
        writeLog('WARN', 'Sem id de mensagem para rastrear ACK — dispatch nao confirmado.');
        return;
    }
...
async function executarFluxoComCliente(client) {
    return new Promise((resolve, reject) => {
        const state = {
            targetMsgId: null,
            ackConfirmado: false,
            acksRecebidos: new Set(),
            finalized: false,
            dispatchIniciado: false
        };
        ...
        client.on('message_ack', (msg, ack) => {
            const msgId = serializarIdMensagem(msg && msg.id);
            if (registrarAckRecebido(state, msgId, ack)) {
                writeLog('INFO', `Evento ACK recebido: ID=${msgId} | Nivel=${ack}`);
            }
        });
        ...
        client.on('ready', async () => {
            ...
            try {
                ...
                if (!chatId.endsWith('@g.us')) {
                    ...
                    const warmup = await client.sendMessage(chatId, '\u231b _Iniciando transmissao do relatorio..._');
                    // Marca o warmup como consumido: sem isso, o message_create dele
                    // continuaria elegivel para o dispatch seguinte quando nao houver
                    // corpo esperado para desambiguar (ver extrairIdMensagemCriada).
                    registrarIdConsumido(serializarIdMensagem(warmup && warmup.id));
                    await sleep(2000);
                }

                const destinosAceitos = await resolverDestinosDoChat(client, chatId);
                const opcoesMensagem = await resolverMencoes(client, CAPTION);
                let dispatch;
                // A partir daqui um erro nao pode mais ser tratado como falha de bootstrap:
                // o sendMessage do conteudo real esta prestes a comecar, e um retry cegamente
                // baseado em regex de mensagem duplicaria o envio (ver shouldRetryInitialize).
                state.dispatchIniciado = true;
                if (ATTACHMENT_PATH && fs.existsSync(ATTACHMENT_PATH)) {
                    ...
                    dispatch = await enviarComConfirmacaoDeDispatch(client, chatId, media, opcoesMensagem, destinosAceitos);
                } else {
                    writeLog('WARN', 'Anexo ausente ou nao encontrado. Enviando apenas a mensagem de texto.');
                    dispatch = await enviarComConfirmacaoDeDispatch(client, chatId, opcoesMensagem.caption, opcoesMensagem, destinosAceitos);
                }
                // Sem ID não há confirmação de despacho. Não commitamos o state
                // nem declaramos sucesso: o ciclo seguinte deve reavaliar a OB.
                definirTargetMsgId(state, dispatch.msgId);
                if (!state.targetMsgId) {
                    throw new Error('WhatsApp não confirmou o despacho da mensagem (sendMessage sem id).');
                }
                ...
                if (state.dispatchIniciado && error && typeof error === 'object') {
                    error.dispatchTentado = true;
                }
                await finalizeAndReject(error);
            }
        });
...
async function executarFluxoBatch(client, itens, chatId, bootstrapAttempt) {
    ...
    // Resolvido uma vez por lote: o endereco canonico do chat nao muda entre fases.
    const destinosAceitos = await resolverDestinosDoChat(client, chatId);
    const resultados = [];
    ...
    for (const [indice, item] of itens.entries()) {
        ...
        try {
            ...
            // ACK e apenas telemetria, nunca criterio de sucesso: em producao o WhatsApp
            // entrega a mensagem despachada mesmo sem emitir message_ack rastreavel, e
            // reenviar apos ACK ausente nunca recuperou nada (0 sucessos em dezenas de
            // ocorrencias). O sucesso da fase e decidido pelo dispatch confirmado abaixo —
            // retorno de sendMessage OU o evento message_create (id as vezes recuperado do
            // campo minificado `$1`, ver serializarIdMensagem) — e SEM essa confirmacao a
            // fase e reprovada por excecao, ACK tendo chegado ou nao.
            // `acksRecebidos` e' por fase (criado e descartado junto com o ackListener)
            // para nao vazar id de uma fase para a seguinte.
            const ackState = { targetMsgId: null, ackConfirmado: false, acksRecebidos: new Set() };
            const ackListener = (msg, ack) => {
                registrarAckRecebido(ackState, serializarIdMensagem(msg && msg.id), ack);
            };
            client.on('message_ack', ackListener);

            try {
                const dispatch = await enviarComConfirmacaoDeDispatch(client, chatId, media, opcoesMensagem, destinosAceitos);
                // `waitUntilMsgSent` e' aplicado por enviarComConfirmacaoDeDispatch; quando
                // a versão instalada omite o ID no retorno, message_create fornece a
                // confirmação equivalente.
                const msgId = dispatch.msgId;
                if (!msgId) {
                    throw new Error('WhatsApp não confirmou o despacho da fase (sendMessage sem id).');
                }
                success = true;

                definirTargetMsgId(ackState, msgId);
...
// Tentativa direta (rapida): mesmo caminho interno que window.WWebJS.getChats() usa hoje
// na 1.34.7. Retorna null (em vez de lancar) quando as colecoes nao aparecem no prazo, para
// o chamador cair no fallback de captura por eventos sem tratar isso como erro fatal.
async function listarGruposViaCollections(client) {
    try {
        await client.pupPage.waitForFunction(
            ...
            { timeout: LIST_GROUPS_COLLECTIONS_TIMEOUT_MS, polling: 250 }
        );
    } catch (_) {
        return null;
    }
...
// Fallback: descobre grupos por eventos de protocolo (message_create + notificacoes de
// grupo), que chegam independentemente do Store/Collections — o mesmo caminho que ja
// confirma o despacho de sendMessage quando o retorno normal falha (ver aguardarEventoDeDespacho).
// Sem enumeracao instantanea disponivel, a contrapartida e' precisar de atividade real no
// grupo durante a janela de captura; grupos sem nenhuma atividade nao aparecem.
function capturarGruposPorEventos(client, duracaoMs) {
    return new Promise((resolve) => {
        const grupos = new Map();
        const registrar = (id) => {
            if (typeof id !== 'string' || !id.endsWith('@g.us')) { return; }
            if (!grupos.has(id)) { grupos.set(id, { id, name: '' }); }
        };
        const onMessage = (msg) => registrar(msg && msg.id && msg.id.remote);
        const onNotification = (n) => registrar(n && n.chatId);

        client.on('message_create', onMessage);
        client.on('group_join', onNotification);
        client.on('group_leave', onNotification);
        client.on('group_admin_changed', onNotification);
        client.on('group_update', onNotification);

        setTimeout(() => {
            client.removeListener('message_create', onMessage);
            client.removeListener('group_join', onNotification);
            client.removeListener('group_leave', onNotification);
            client.removeListener('group_admin_changed', onNotification);
            client.removeListener('group_update', onNotification);
            resolve(Array.from(grupos.values()));
        }, duracaoMs);
    });
}
...
async function executarModoListGroups() {
        ...
        client.on('ready', async () => {
            try {
                let groups = await listarGruposViaCollections(client);
                if (!groups) {
                    writeLog('WARN', 'window.require(\'WAWebCollections\') indisponivel ou vazio '
                        + `apos ${LIST_GROUPS_COLLECTIONS_TIMEOUT_MS}ms — capturando grupos por `
                        + `eventos de protocolo por ${LIST_GROUPS_CAPTURE_MS}ms (envie ou gere `
                        + 'qualquer atividade nos grupos que deseja listar).');
                    groups = await capturarGruposPorEventos(client, LIST_GROUPS_CAPTURE_MS);
                }
...
module.exports = {
    ...
    aguardarAck,
    serializarIdMensagem,
    chaveIdMensagem,
    registrarAckRecebido,
    definirTargetMsgId,
    registrarIdConsumido,
    _resetIdsConsumidosForTests,
    extrairIdMensagemCriada,
    normalizarDestinos,
    resolverDestinosDoChat,
    executarFluxoComCliente,
    processar,
    aguardarAckBatch,
    executarFluxoBatch,
    executarModoListGroups,
    LIST_GROUPS_CAPTURE_MS,
    executarModoBatch,
