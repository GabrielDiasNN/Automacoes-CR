...
// Modo batch: settle e ACK adaptativos por número de tentativas de bootstrap
const BATCH_SETTLE_MS_NORMAL = 20000;  // 20s quando bootstrap ok na 1ª tentativa
const BATCH_SETTLE_MS_RETRY  = 40000;  // 40s após retry — conexão demonstrou instabilidade
const ACK_WAIT_ATTEMPTS_BATCH_NORMAL = 30; // 30 × 2s = 60s por mensagem (normal)
const ACK_WAIT_ATTEMPTS_BATCH_RETRY  = 50; // 50 × 2s = 100s por mensagem (após retry)
...
async function executarFluxoBatch(client, itens, chatId, bootstrapAttempt) {
    ...
    for (const item of itens) {
        ...
        try {
            ...
            } else if (bootstrapAttempt === 1) {
                // Bootstrap estavel: ACK pode ter chegado apos o timeout (servidor lento)
                // Marcamos sucesso para evitar duplicata na proxima execucao
                success = true;
                writeLog('WARN', `[BATCH] ${phase_key}: ACK nao confirmado mas bootstrap foi estavel — assumindo entrega.`);
            } else {
                // Bootstrap instavel (precisou de retry): sem ACK = falha real
                // A proxima execucao vai reenviar esta fase
                success = false;
                writeLog('WARN', `[BATCH] ${phase_key}: ACK nao confirmado apos bootstrap instavel — marcando como FALHA para reenvio.`);
            }
        } catch (err) {
