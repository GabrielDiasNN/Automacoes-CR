    ...
    for (let i = 1; i <= ACK_WAIT_ATTEMPTS; i++) {
        ...
        if (client.pupPage) {
            try {
                ...
                writeLog('WARN', `Keep-alive do browser nao respondeu nesta iteracao: ${error.message}`);
            }
        }
    ...
    return new Promise((resolve, reject) => {
        ...
        client.on('ready', async () => {
            try {
                ...
                writeLog('ERROR', `Erro no protocolo Soberano: ${error.message}`);
                await finalizeAndReject(error);
            }
        });
        ...
        client.initialize().catch(async (error) => {
            writeLog('ERROR', `Erro na inicializacao do WhatsApp: ${error.message}`);
            await finalizeAndReject(error);
        });
    });
}
    ...
    for (let attempt = 1; attempt <= INITIALIZE_MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (shouldRetryInitialize(error) && attempt < INITIALIZE_MAX_ATTEMPTS) {
                writeLog('WARN', `Falha transiente na inicializacao do WhatsApp: ${error.message}. Retentando em ${INITIALIZE_RETRY_DELAY_MS}ms...`);
...
(async () => {
    try {
        ...
        if (e && e.message && e.message.includes('No LID for user')) {
            writeLog('WARN', `Contato invalido / No LID: ${e.message}. Encerrando graciosamente sem falhar a automacao.`);
            process.exit(0);
        } else {
            writeLog('ERROR', `ENCERRAMENTO POR FALHA: ${e.message}`);
            process.exit(24);
        }
    }
})();
