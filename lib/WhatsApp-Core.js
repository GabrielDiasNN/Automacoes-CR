...
async function resolverMencoes(client, caption) {
    ...
    for (const numero of candidatos) {
        try {
            const wid = await comTimeout(
                client.getNumberId(numero),
                MENTION_RESOLVE_TIMEOUT_MS,
                `Timeout (${MENTION_RESOLVE_TIMEOUT_MS}ms) ao resolver ${numero}`
            );
            if (!wid) {
                writeLog('WARN', `Numero ${numero} nao registrado no WhatsApp (getNumberId=null) — mencao ignorada.`);
                continue;
            }
            ...
            writeLog('WARN', `Falha ao resolver mencao para ${numero}: ${describeError(error)}`);
        }
    }
...
async function executarFluxoBatch(client, itens, chatId, bootstrapAttempt) {
    ...
    for (const item of itens) {
        ...
        try {
            ...
            // Reenviar o mesmo conteudo apos ACK ausente nunca confirmou entrega em nenhum caso
            // do historico de producao (0 sucessos em dezenas de ocorrencias) — o reenvio so
            // soma 120-150s de espera extra sem chance real de recuperar. Falha rapido aqui;
            // o proximo ciclo do cron tenta de novo com conteudo atualizado.
            ...
            try {
                ...
                if (acked) {
                    ...
                    writeLog('WARN', `[BATCH] ${phase_key}: ACK nao confirmado — marcando falha sem reenvio.`);
                    success = false;
                }
            } finally {
