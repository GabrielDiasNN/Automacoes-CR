...
// Modo batch usa timeout menor: mensagem já está na fila local, ACK é confirmação
const ACK_WAIT_ATTEMPTS_BATCH = 20; // 20 × 2s = 40s por mensagem
...
async function aguardarAckBatch(client, state, phaseKey) {
    writeLog('INFO', `[BATCH] ${phaseKey}: Aguardando ACK (até ${ACK_WAIT_ATTEMPTS_BATCH * ACK_WAIT_DELAY_MS / 1000}s)...`);
    for (let i = 1; i <= ACK_WAIT_ATTEMPTS_BATCH; i++) {
        if (state.ackConfirmado) {
    ...
    writeLog('WARN', `[BATCH] ${phaseKey}: ACK nao confirmado em ${ACK_WAIT_ATTEMPTS_BATCH * ACK_WAIT_DELAY_MS / 1000}s — mensagem esta na fila local do WhatsApp.`);
    return false; // nao bloqueia — mensagem foi enviada para a fila
}

async function executarFluxoBatch(client, itens, chatId) {
    const resultados = [];
    ...
    for (const item of itens) {
        ...
        try {
            ...
            const acked = await aguardarAckBatch(client, ackState, phase_key);
            success = true; // mensagem enviada — ACK é informativo em modo batch
            if (acked) {
    ...
    for (let attempt = 1; attempt <= INITIALIZE_MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            const resultados = await new Promise((resolve, reject) => {
                ...
                client.on('ready', async () => {
                    writeLog('INFO', `Cliente pronto. Aguardando estabilizacao do WhatsApp Web (20s)...`);
                    await sleep(20000);
                    writeLog('INFO', `Processando ${itens.length} fases em lote...`);
                    try {
                        const res = await executarFluxoBatch(client, itens, chatId);
                        await sleep(ACK_SETTLE_DELAY_MS);
                        await destruirClienteSilenciosamente(client);
                        resolve(res);
                    } catch (err) {
