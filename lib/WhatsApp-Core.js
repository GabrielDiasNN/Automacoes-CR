...
async function healthCheckCanal(client, chatId) {
    writeLog('INFO', '[HEALTH-CHECK] Enviando probe de saude do canal...');
    try {
        const probe = await client.sendMessage(chatId, '⏳ _Verificando canal..._');
        const probeId = probe.id._serialized;
        let acked = false;
        const listener = (msg, ack) => {
            if (msg.id._serialized === probeId && ack >= 1) acked = true;
        };
        client.on('message_ack', listener);
        // Timeout curto: 30s (15 tentativas × 2s)
        for (let i = 0; i < 15 && !acked; i++) {
            await sleep(2000);
        }
        client.removeListener('message_ack', listener);
        if (!acked) {
            writeLog('ERROR', '[HEALTH-CHECK] Probe nao confirmada em 30s. Canal degradado.');
            throw new Error('HEALTH_CHECK_FAILED');
        }
        writeLog('INFO', '[HEALTH-CHECK] Canal confirmado como funcional.');
        return true;
    } catch (e) {
        writeLog('ERROR', `[HEALTH-CHECK] Falha ao enviar probe: ${e.message || e}`);
        throw new Error('HEALTH_CHECK_FAILED');
    }
}
        ...
        try {
            ...
            const resultados = await new Promise((resolve, reject) => {
                ...
                client.on('ready', async () => {
                    ...
                    try {
                        // --- HEALTH CHECK PONTO-A-PONTA ---
                        await healthCheckCanal(client, chatId);
