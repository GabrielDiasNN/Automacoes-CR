...
describe('writeLog', () => {
    ...
    describe('modo estruturado (HUB_LOG_STRUCTURED=1)', () => {
        ...
        test('emite envelope JSON no schema (component=node_whatsapp) e mascara segredo', (t) => {
            ...
            localCore.writeLog('ERROR', 'falha ao conectar token: abcd1234efgh');
            ...
            assert.ok(!evt.message.includes('abcd1234efgh'));
            assert.ok(evt.message.includes('[REDACTED]'));
        });

        test('grava a linha JSON (nao a legada) no LOG_FILE', (t) => {
            ...
            const conteudo = fs.readFileSync(logFile, 'utf8').trim();
            assert.doesNotMatch(conteudo, /\[NODE-WA\]/);
            assert.equal(JSON.parse(conteudo).message, 'mensagem estruturada');
        });
    });
});
