...
describe('executarModoListGroups', () => {
    ...
    test('ready: colecoes disponiveis -> lista grupos e sai com exit(0)', async (t) => {
    ...
    test('ready: colecoes indisponiveis -> cai para captura por eventos e descobre grupos', async (t) => {
        t.mock.timers.enable({ apis: ['setTimeout'] });
        const authPath = fs.mkdtempSync(path.join(SUITE_TMP_ROOT, 'lg-collections-timeout-'));
        criarSessaoValida(authPath, 'lg-collections-timeout');
        const localCore = loadCore({ authPath, clientId: 'lg-collections-timeout', mode: 'LIST_GROUPS', contact: '' });
        const { module: wa, created } = makeWhatsAppMock(() => new FakeClient({
            pupPage: {
                mouse: { move: async () => {} },
                waitForFunction: async () => { throw new Error('timeout WAWebCollections'); },
                evaluate: async () => []
            }
        }));
        localCore._setWhatsAppWebJsForTests(wa);
        ...
        created[0].emit('ready');
        await flushAsync();
        assert.ok(logs.some((l) => l.includes('capturando grupos por eventos de protocolo')));
        created[0].emit('message_create', { id: { remote: 'g2@g.us' } });
        created[0].emit('group_update', { chatId: 'g3@g.us' });
        created[0].emit('message_create', { id: { remote: '5511999999999@c.us' } }); // chat individual: ignorado
        await t.mock.timers.tick(localCore.LIST_GROUPS_CAPTURE_MS);
        await flushAsync();
        assert.equal(exitMock.mock.calls[0].arguments[0], 0);
        // A saida em JSON e' a unica linha parseavel como array — as demais sao logs
        // estruturados ([timestamp]/[NODE-WA]... ou JSON de objeto, nao de array).
        let saida;
        for (const l of logs) {
            try {
                const parsed = JSON.parse(l);
                if (Array.isArray(parsed)) { saida = parsed; break; }
            } catch (_) { /* linha nao e' JSON de array puro, ignora */ }
        }
        assert.ok(saida, 'saida JSON com a lista de grupos nao encontrada nos logs');
        assert.deepEqual(saida.map((g) => g.id).sort(), ['g2@g.us', 'g3@g.us']);
    });
