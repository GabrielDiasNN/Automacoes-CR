...
function Write-RunLog {
    ...
    if (Get-Command Write-AutomacaoLog -ErrorAction SilentlyContinue) {
        ...
        # Traduzir niveis para os suportados pela Lib-Logging
...
function Remove-DirectorySafe {
    ...
    try {
        ...
        if ($DryRun) {
            ...
            Write-RunLog -Level "INFO" -Message ("[DRY-RUN] Remover diretorio ({0}): {1} | arquivos={2}" -f $Reason, $DirectoryPath, $files.Count)
        ...
        Write-RunLog -Level "INFO" -Message ("Diretorio removido ({0}): {1}" -f $Reason, $DirectoryPath)
...
function Remove-OldLogFiles {
    ...
    if (-not (Test-Path -LiteralPath $DirectoryPath)) {
        ...
        Write-RunLog -Level "INFO" -Message ("Diretorio inexistente ({0}): {1}" -f $Reason, $DirectoryPath)
...
try {
    ...
    # Limpar logs diarios legados (log_DD-MM-YYYY.log, Execution.log, VBA_Internal.log)
    ...
    # Safety net: remover apenas se nao modificados ha 15+ dias (rotacao de conteudo e feita pelo run.ps1)
    ...
    # Arquivo legado ReceitasBloqueadas.txt na raiz do modulo
    ...
    # Monitor logs -> politica padrao (KeepLogsDays)
    Remove-OldLogFiles -DirectoryPath (Join-Path $BasePath "Logs") -Cutoff $logsCutoff -Reason "retencao-monitor-log" -Filter "Monitor_*.log"

    # Logs de retencao proprios -> politica de auditoria (KeepAuditDays)
    Remove-OldLogFiles -DirectoryPath (Join-Path $BasePath "Logs") -Cutoff $auditCutoff -Reason "retencao-logs-retencao" -Filter "Retention_*.log"
            ...
            try {
                ...
                if (-not $hasFiles) {
                    ...
                    if ($DryRun) {
                        ...
                        Write-RunLog -Level "INFO" -Message ("[DRY-RUN] Remover diretorio vazio (retencao-audit): {0}" -f $dir.FullName)
                        ...
                        Write-RunLog -Level "INFO" -Message ("Diretorio vazio removido (retencao-audit): {0}" -f $dir.FullName)
                ...
                Write-RunLog -Level "WARN" -Message ("Falha ao remover diretorio vazio no Audit: {0} | Erro: {1}" -f $dir.FullName, $_)
