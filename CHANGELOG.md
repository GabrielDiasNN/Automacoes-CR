...
- **`.gitignore` cobria só `.env` exato**: um `.env.local` ou `.env.producao` — nome comum de variante por máquina — entrava no commit sem aviso, num repositório cujo histórico já precisou ser reescrito por PII. Passou a ignorar `.env.*` com exceção explícita para `.env.example` (verificado que este continua versionado). Na mesma passagem entraram `.claude/.state/` (carimbos de última execução dos gates, estado por máquina que aparecia como untracked a cada commit) e `.benchmarks/`; `.ruff_cache/` e `.hypothesis/` foram declarados por escrito — hoje se autoprotegem com um `.gitignore` gerado dentro do próprio cache, o que é proteção emprestada de terceiro.
