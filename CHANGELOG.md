...
- **⚠️ Mudança de formato do hash de idempotência**: o hash persistido em `receitas_state.json` muda de `pd.util.hash_pandas_object(...).sum()` para SHA-256 determinístico (`compute_hash()`). Isso é uma mudança de contrato operacional: a primeira execução pós-deploy vai comparar um hash em formato novo contra `email_state.json.last_sent_hash` (formato antigo), disparando reenvio único de e-mail/WhatsApp mesmo sem mudança real de dados. Mitigação: deploy só deve ocorrer com a janela "assentada" (`delivery_status.email.success`/`.whatsapp.success` = true para o hash atual, sem retry pendente) — documentado em `docs/runbooks/receitas-bloqueadas-runbook.md`.
