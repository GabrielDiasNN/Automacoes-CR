...
``contracts.analytics``. Este módulo concentra apenas builders SQL reutilizados
pelos três contratos, evitando dependência do antigo módulo de compatibilidade.
"""
...
from ..data.schema import COLUMN_NAMES, TABLE_NAME, init_db

# Projecao tipada usada no drill-down (tudo menos o blob de auditoria).
_DETAIL_TYPED: tuple[str, ...] = tuple(
    c for c in COLUMN_NAMES if c != "DADOS_COMPLETOS"
)
_DETAIL_SELECT = ", ".join(_DETAIL_TYPED)

# Aliases finos para preservar os nomes internos usados no modulo.
...
def _detail_row_to_record(row: sqlite3.Row) -> dict[str, Any]:
    """Converte uma linha tipada em registro, espelhando os rotulos de turno."""
...
def _detail_raw_records(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Reconstroi os payloads originais a partir do blob (apenas em include_raw)."""
