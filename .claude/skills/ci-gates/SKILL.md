...
- **08/09/2026 — `docs/templates` entrou em ruff e bandit.** `automation-smoke-test-template.py` é `.py` executável (fonte que `Tools/New-Automation.ps1` copia no scaffold de automação nova) que ficou fora do inventário de escopo por lacuna, não por decisão — já passava limpo em ambas as ferramentas antes da inclusão.
