...
if ([string]::IsNullOrWhiteSpace($ExecId)) {
    $ExecId = if (Get-Command Register-ExecutionTelemetry -ErrorAction SilentlyContinue) {
        Register-ExecutionTelemetry -AutomationName "OBs Restrição Branco"
...
Write-Log "INICIO — OBs Restrição Branco (ORB-07). ExecId=$ExecId"
    ...
    try {
        ...
        if ($pyResult.Idempotent) {
            if (Test-Path $StateTmp) {
                ...
                Write-Log "State reconciliado sem necessidade de envio."
            }
