...
def test_broadcast_endpoints_rejeitam_sem_api_key(client):
    for path, payload in BROADCAST_CASES:
        response = client.post(path, json=payload)
        assert response.status_code == 403, f"{path} deveria exigir API Key"


def test_broadcast_endpoints_rejeitam_api_key_invalida(client):
    for path, payload in BROADCAST_CASES:
...
def test_broadcast_endpoints_aceitam_api_key_valida(client):
    for path, payload in BROADCAST_CASES:
