...
def _collect_portfolio_rows(
    ...
    latest_lookup = get_latest_execution_snapshot_by_automation(db, automation_ids)
    ...
    for auto in automations:
        ...
        automation_response_by_id[auto_id] = build_automation_response(
            ...
            latest_execution_lookup=latest_lookup,
            metrics_24h_lookup=metrics_lookup,
        )
    ...
    for entry in manifest_entries:
        ...
        response = (
            automation_response_by_id.get(int(matched_auto.id))
            if matched_auto
            else None
        )
        latest_snapshot = (
    ...
    for auto in automations:
        if int(auto.id) in matched_automation_ids:
            continue
        response = automation_response_by_id[int(auto.id)]
        latest_snapshot = latest_lookup.get(int(auto.id), {})
        last_success = last_success_lookup.get(int(auto.id))
        last_failure = last_failure_lookup.get(int(auto.id))
        issues = [
            _issue(
                "manifest_missing",
...
def read_portfolio_runbook(
    ...
    for entry in load_catalog_manifests(root, include_template=False):
        manifest = entry.manifest
        if manifest.id != catalog_id and manifest.slug != catalog_id:
            continue
